...
contract Streamer is Ownable {
    ...
    error ChannelAllreadyRunningError();
    error NoOpenChannelForAddress();
    error ChannelBalanceInsufficient();
    error NoClosingChannelForAddress();
    error ChannelChallengeNotComplete();
    ...
    function fundChannel() public payable {
        ...
        */
        if(balances[msg.sender] != 0){
            revert ChannelAllreadyRunningError();
        }
    ...
    function withdrawEarnings(Voucher calldata voucher) public {
        ...
        */
        ...
        uint256 balance = balances[signer];

        if(!(balance > 0)){
            revert NoOpenChannelForAddress();
        }
        if(balance <= voucher.updatedBalance){
            revert ChannelBalanceInsufficient();
        }

        uint256 payment = voucher.updatedBalance ;

        balances[signer] = balance - payment;
    ...
    */
    function defundChannel() public {
        if(!(canCloseAt[msg.sender] > 0)){
            revert NoClosingChannelForAddress();
        }
        if(canCloseAt[msg.sender] > block.timestamp){
            revert ChannelChallengeNotComplete();
        }
